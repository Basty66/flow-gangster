...
const sizesIcon = (
  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <path d="M4 20h16"/>
    <path d="M9 16l-5-5 5-5"/>
    <path d="M15 16l5-5-5-5"/>
  </svg>
);
...
export default function ProductCard({ producto, index = 0 }) {
  ...
  return (
    <Link to={`/producto/${producto.id}`}
          className="glass-card-hover block group animate-fade-in-up"
      ...
      <div className="relative aspect-square overflow-hidden bg-[#111111]">
        ...
        {isLowStock && (
          <div className="absolute top-3 right-3">
            <span className="bg-amber/10 text-amber text-[9px] font-bold px-2 py-1 uppercase tracking-[0.1em] border border-amber/20">
              LOW STOCK
            </span>
          </div>
        )}
      ...
      <div className="p-5 space-y-2.5">
        <div className="flex items-center justify-between">
          ...
          {producto.modalidad === 'STOCK' && (
            <span className="text-[#525252] text-[10px] font-medium">SKU: {producto.id?.slice(0, 6).toUpperCase()}</span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-white/5">
          <p className="font-display font-extrabold text-xl tracking-tight text-accent">
