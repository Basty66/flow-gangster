    ...
    <div className="min-h-screen">
      ...
      <section className="relative min-h-[80vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto pt-16">
          ...
          <h1 className="font-display font-black text-6xl sm:text-7xl md:text-8xl lg:text-9xl leading-[0.85] tracking-[-0.04em] mb-6 animate-fade-up">
            <span className="text-[#f5f5f5]">NEXT</span>
            <br />
            <span className="text-purple">DROP</span>
          </h1>

          <p className="text-[#666] text-base md:text-lg max-w-lg mx-auto mb-10 font-body leading-relaxed animate-fade-up"
          ...
          <div className="flex justify-center gap-12 md:gap-16 mt-16 animate-fade-up"
               style={{ animationDelay: '0.3s', opacity: 0 }}>
            <div>
              <p className="font-display font-black text-3xl text-[#f5f5f5]">{productos.filter(p => p.modalidad === 'STOCK').length}</p>
              <p className="text-[#666] text-xs font-medium tracking-[0.1em] uppercase mt-1">Instant Drop</p>
            </div>
            <div>
              <p className="font-display font-black text-3xl text-[#f5f5f5]">{productos.filter(p => p.modalidad === 'ENCARGO').length}</p>
              <p className="text-[#666] text-xs font-medium tracking-[0.1em] uppercase mt-1">Pre-Order</p>
            </div>
            <div>
              <p className="font-display font-black text-3xl text-[#f5f5f5]">{marcas.length}</p>
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#444" strokeWidth="1.5" className="animate-bounce">
      ...
      <section id="catalogo" className="max-w-7xl mx-auto px-4 py-20">
          ...
          <div className="flex flex-wrap gap-1.5">
            {[
              ...
              <button key={f.key} onClick={() => setFiltro(f.key)}
                      className={`px-4 py-2 rounded text-xs font-medium tracking-[0.08em] uppercase transition-all ${
                        filtro === f.key
        ...
        {!loading && !error && filtrados.length === 0 && (
          <div className="text-center py-20 space-y-4 max-w-sm mx-auto">
            ...
            {busqueda && (
              <button onClick={() => setBusqueda('')} className="btn btn-outline">Limpiar</button>
            )}
          </div>
        )}
