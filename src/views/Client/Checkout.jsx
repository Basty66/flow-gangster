      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="flex items-center justify-center gap-2 mb-12">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center">
              <div className={`stepper-dot ${step >= s ? 'border-accent text-accent' : 'border-white/10 text-[#525252]'}`}>
                {step > s ? <CheckIcon /> : s}
              </div>
              {s < 3 && <div className={`w-10 h-[1px] ${step > s ? 'bg-accent' : 'bg-white/10'}`} />}
            </div>
          ))}
        </div>
            ...
            <div className="space-y-3">
              {items.map((item, idx) => (
                <div key={idx} className="glass-card p-4 flex justify-between items-center">
                  ...
                  <p className="font-bold text-sm text-accent">${(item.producto.precio * item.cantidad).toLocaleString('es-CL')}</p>
                </div>
              ))}
            </div>
            <div className="text-right">
              <p className="text-[#525252] text-sm">Subtotal: ${totalPrecio.toLocaleString('es-CL')}</p>
              <p className="font-display font-extrabold text-xl text-accent">TOTAL: ${totalPrecio.toLocaleString('es-CL')}</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-3">
                  <button onClick={() => update('tipo_entrega', 'RETIRAR_SHOWROOM')}
                          className={`flex-1 py-3.5 border font-bold text-xs uppercase tracking-wider transition-all ${
              ...
              {form.tipo_entrega === 'ENVIO_STARKEN' && (
                <div className="glass-card p-5 space-y-4 animate-fade-in">
                  <p className="font-bold uppercase text-xs tracking-wider text-accent">Direccion de envio</p>
        ...
        {step === 3 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="glass-card p-5 space-y-2">
              ...
              {form.tipo_entrega === 'ENVIO_STARKEN' && (
                <p><span className="text-[#525252] text-sm">Direccion:</span> <span className="font-bold">{form.direccion}</span></p>
              )}
              <p><span className="text-[#525252] text-sm">Pago:</span> <span className="font-bold">{form.metodo_pago === 'TRANSFERENCIA' ? 'Transferencia Bancaria' : 'Efectivo al Retirar'}</span></p>
              {form.cupon && <p><span className="text-[#525252] text-sm">Cupon:</span> <span className="font-bold text-accent">{form.cupon}</span></p>}
            </div>

            <div className="glass-card p-5">
              <p className="font-display font-extrabold text-2xl text-accent">TOTAL: ${totalPrecio.toLocaleString('es-CL')}</p>
            </div>
        ...
        {step === 4 && resultado && (
          <div className="text-center space-y-6 animate-scale-in">
            <div className="w-16 h-16 rounded-full border-2 border-accent flex items-center justify-center mx-auto text-accent">
              <CheckIcon />
            </div>
            ...
            {form.metodo_pago === 'TRANSFERENCIA' && (
              <div className="glass-card p-6 max-w-md mx-auto space-y-3">
                <p className="font-bold text-amber uppercase tracking-wider text-xs">
                  IMPORTANTE: Tienes 2 horas para transferir
                </p>
                ...
                <p className="font-bold text-lg">Cuenta Corriente: ___</p>
                <p className="font-bold text-accent">Alias: FLOW.GANGSTER</p>
